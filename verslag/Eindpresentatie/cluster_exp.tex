
\begin{frame}{Cluster Expansions}

    \begin{minipage}{0.5\textwidth}

        % \only<1> {
        %     \begin{equation}
        %         e^{H(1)} =  \vcenter{ \hbox{ \paa  }}
        %     \end{equation}
        %     \begin{equation}
        %         \begin{split}
        %             e^{H(2)} =&  \vcenter{ \hbox{ \pa }} \\
        %             +&  \vcenter{ \hbox{ \pb }}
        %         \end{split}
        %     \end{equation}

        % }

        \only<1-4> { \clustFullA }
        \only<5>{
            \begin{itemize}
                \item Multiple choices for encoding
                \item Size extensive
                \item Preserves global and internal symmetries
                \item Tensor Network toolbox
                      % \item Solvers
                      %       \begin{itemize}
                      %           \item Linear
                      %           \item Nonlinear
                      %       \end{itemize}

            \end{itemize}
        }
    \end{minipage}
    \begin{minipage}{0.49\textwidth}

        \only<1-3> {
            \begin{itemize}
                \item   $e^{ - \beta \hat{H} } = \sum_{ \{B\} }  \bigotimes_i B_i $

                      \only<1>{

                          \begin{equation*}
                              e^{  - \beta H(1)} =  \vcenter{ \hbox{ \paa  }}
                          \end{equation*}

                          \begin{equation*}
                              \begin{split}
                                  e^{  - \beta H(2)} =&  \vcenter{ \hbox{ \pa }} \\
                                  +&  \vcenter{ \hbox{ \pb }}
                              \end{split}
                          \end{equation*}

                      }

                      \only<2>{
                          \begin{equation*}
                              \begin{split}
                                  e^{ - \beta H(3) } =&  \vcenter{ \hbox{ \pcc }} \\
                                  +&  \vcenter{ \hbox{ \pca }} \\
                                  +&  \vcenter{ \hbox{ \pcb }} \\
                                  +&  \vcenter{ \hbox{ \pc }}
                              \end{split}
                          \end{equation*}
                      }

                      \only<3>{
                \item Finite number of blocks:

                      trucate order
                \item Encoded by 1 tensor
                      \begin{equation*}
                          O^{abcd} = \vcenter{ \hbox{ \pepob{4}{3}{{
                                              "-","-","-",
                                              "-","a","c",
                                              "-","-","-"}}{{
                                              "-","-",
                                              "-","-",
                                              "d","b",
                                              "-","-"}}{{
                                              1,1,4,1,
                                              1,4,12,4,
                                              1,1,4,1}} }}
                      \end{equation*}
                      }
            \end{itemize}
        }

        \only<4-> {
            \clustFullB
        }

    \end{minipage}

    \only<2->{  \addtocounter{framenumber}{1} }

\end{frame}

