...
The  blocks $O^{n-1,n}$ and $O^{n,n-1}$ are unitary matrices from the \Gls{SVD} decomposition, scaled by the largest singular value. The blocks $O^{n-1,n'}$ and $O^{n',n-1}$ are then used to actually solve the problem for the given chain. In the next step, $O^{n,n}$ is added as usual. The idea here is once again to keep this block small, in order to not cause any divergences.
