...
\section{Quantum to classical mapping}
...
\begin{equation}
    \begin{split}
        Z &= \sum e^{ - \beta E_n} \\
        &= \sum_n \Braket{n | e^{ - \beta \hat{H} }  | n} \\
        &= \sum_{n_1 \cdots n_M} \Braket{n_1 | e^{ - \beta \hat{H} /M}  | n_2}  \Braket{n_2 | e^{ - \beta \hat{H} /M }  | n_3} \cdots  \Braket{n_{M-1} | e^{ - \beta \hat{H} /M }  | n_M} \\
        &= \sum_{  n_1 \cdots n_M} \Braket{n_1 | e^{ - \beta \hat{H} /M}  | n_2}  \Braket{n_2 | e^{ - \beta \hat{H} /M }  | n_3} \cdots  \Braket{n_{M-1} | e^{ - \beta \hat{H} /M }  | n_M} \\
    \end{split}
\end{equation}
This result applies M MPO layers, and hence this is a tensor network with 1 higher dimension.
\todo{improve}