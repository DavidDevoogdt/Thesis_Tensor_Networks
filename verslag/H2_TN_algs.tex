...
A translation invariant MPS has the form shown in \cref{mps:uni} It can easily be seen that inserting $X X^{-1}$ on each bond doesn't change the contracted tensor. This is called a gauge transformation. Defining
\begin{equation}
    \begin{split}
        &A_l = \pepob{3}{2}{{
                    "-","-",
...
Time-evolving block decimation was introduced as a method to calculate thermal states (this will also be encountered later on in \cref{rt_tn_methods}). It can also be used as a power method for contraction.
