...
It can be easily seen that inserting $X X^{-1}$ on each bond doesn't change the contracted tensor. This is called a guage transformation.
\begin{equation}
    A_l = \pepob{3}{2}{{
                "-","-",
                "",""}}{{
...
One properties of MPS is that expectation values can be calculated exactly. suppose we have an operator $\hat{O}$ acting on a single site of the MPS: $\Braket{ \Psi |  \hat{O} | \Psi  }$
\begin{equation}
    \vcenter{\hbox{\pepob{5}{3}{{
                        ...
                        1,4,7,4,1}} }}
\end{equation}
where we made use of \cref{algs:mpsid}. Similar techniques allow to calculate correlatetion functions, energies, ...
...
Suppose that the there is an MPO representation A that gives the probability of a state, and an operator X which performs a measurement over a number of sites. The expecatation value $ \Braket{X}$ is given by:
\begin{equation}
    \Braket{X} = \frac{
        \begin{tikzpicture} [   ]
...
\end{equation}
This problem is much harder than in 1D. For instance, the related problem of calculating the expecatation value of on operator, similar to \cref{subsec:exp_val_1d} in 1D, is  \#P-Hard \cite{Orus2014}.
...
In \cite{Nietner2020}, a version of VUMPS is proposed that can compute the environment of a m by n unit cell with only a linear increase in computation cost. Larger unit cells might be needed for groundstates with non-trivial unit cell .
...
\begin{equation}
    \vcenter{ \hbox{   \pepob{5}{3}{{
                        ...
                        1,4,16,4,1,
                        1,4,0,4,1,
                        1,5, 18  ,1,1}} }}  = \lambda   \vcenter{ \hbox{  \pepob{3}{2}{{
                        "","",
                        "-","-"}}{{
                        "-",
                        "",
                        ...
                        4,4,4}} }}
\end{equation}
The idea is to use the same environments $G_l$ and $G_r$, but find the fixed point from below. This has some problems: $B$ cannote be brought into a canonical form according to \cref{algs:ACC} (in combination with the analogue of \cref{algs:vumpsenvc}), and hence does not represent a fixed point MPS as was the case from above. This also doesn't work in the multisite setting.

This is not completely surprising, suppose we have a fixed point MPS $B$ (denoted in bold) from below (for instance obtained by performing the VUMPS algorithm on an MPO rotated by 180 degrees), then contracting the upper half plane and the lower half plane amounts to:
...
\begin{equation}\label{eq:vumps_B_own}
    \vcenter{ \hbox{   \pepob{3}{2}{{
                        ...
                        "-"}}{{
                        24, 22 ,4,
                        4,  2 ,4}} }} = \lambda    \vcenter{ \hbox{  \pepob{3}{2}{{
                        "","",
                        "",""}}{{
                        "",
                        "-",
