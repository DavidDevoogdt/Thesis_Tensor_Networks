...
function phase_transition()
    ...
    beta_arr = exp( -1:0.1:2);
    t_arr = 1./beta_arr;
    
    beta_len = size(beta_arr,2);
    
    gamma_arr = -2.11:0.2:3.6;
    gamma_len = size(gamma_arr,2);
    
    results = zeros(gamma_len,beta_len);
    ...
    for beta_i = 1:beta_len
       ...
       for gamma_i = 1:gamma_len
           ...
           %simul.Order = 5;
           %MPO = generateMPO(d,-beta*H_1_tensor,-beta*H_2_tensor,3,simul.Order,opt3,generate_opts);
           
           simul.Order = 3;
           MPO = generateMPO(d,-beta*H_1_tensor,-beta*H_2_tensor,4,simul.Order,opt4,generate_opts);
           
           MPO_matrix = MPO.MPO_cell * MPO.nf;
           
%            m = get_expectation(MPO_matrix, S_z_2);
%            results(gamma_i,beta_i) = m ;
           
           [A, xi] =   get_correlation_length(MPO_matrix, S_z_2,4);
           fprintf(" %.4e ",xi);
           results(gamma_i,beta_i) = xi ;
