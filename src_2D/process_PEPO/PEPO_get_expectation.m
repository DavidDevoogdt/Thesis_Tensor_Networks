...
function [results, save_vars] = PEPO_get_expectation (X, save_vars, vumps_opts, results, opts)
    ...
    if vumps_opts.cell_size == 1
        ...
        if doSym 
            ...
            Bl = Ar;
            Bl.var =  ncon({ Bl.var }, {[-3,-2,-1]});

            Br = Al;
            Br.var =  ncon({ Br.var }, {[-3,-2,-1]});

            Bc = Ac;    
            Bc.var = ncon({ Bc.var }, {[-3,-2,-1]});


            %overlap between A and B
            xtens.group = 'none';
            xtens.legs = 2;
            xtens.var = rand( vumps_opts.chi_max );
            ...
            v =  TensorContract(  {fl,Bc,fr},{ [-1,1],[1,-2,2],[-3,2] });
        else
            [v, ~, ~] = TensorEigs(@(x) get_Bc(x, GL, GR), TensorConj(Ac), 1, 'lm', opts);
        end
        

        [rho, ~] = TensorContract({v, GL, Ac, o2, GR}, ...
