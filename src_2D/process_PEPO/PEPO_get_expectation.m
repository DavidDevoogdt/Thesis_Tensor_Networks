...
function [results, out_vars] = PEPO_get_expectation (X, in_vars, vumps_opts)

    assert(isfield(in_vars, 'PEPO_matrix'));
    T = in_vars.PEPO_matrix;

    if ~isfield(in_vars, 'A') ||~isfield(in_vars, 'G0')
