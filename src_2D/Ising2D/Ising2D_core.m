function [m_arr, T_arr, corr_arr, marek_arr, ctr_arr, vumps_err_arr] = Ising2D_core(name_prefix,  J, g, onsager, chi, i, m_arr, T_arr, corr_arr, marek_arr, ctr_arr, vumps_err_arr)
    ...
    parfor iii = 1:numel( T_arr(index,:) )
        ...
        if onsager == 1%for testing purposes
            ...
            marek_arr_2( index) = 1;
            ctr_arr_2( index ) = 0;
            vumps_err_arr_2( index ) = 0;

            pause(1)
        else
            pepo = PEPO(d, -beta(iii) * H_1_tensor, -beta(iii) * H_2_tensor, 5, handle, opts);
            [mm, inv_corr_length, marek_arr_2(index), ctr_arr_2(index), vumps_err_arr_2( index)] = PEPO_get_expectation(pepo, S_z, chi);
        ...
        %temp_save in folder
        saveboy(sprintf("%s/temp_%d.mat", name_prefix, iii), 'T_arr_2', 'm_arr_2', 'corr_arr_2', 'marek_arr_2', 'vumps_err_arr_2', 'ctr_arr_2','index', 'iii' , T_arr_2, m_arr_2, corr_arr_2, marek_arr_2,  vumps_err_arr_2, ctr_arr_2,index,iii);
        fprintf("%s %3d:%2d:%2d T:%.4e mag:%.4e xi:%.4e marek gap:%.4f ctr:%3d err:%.4e\n", datestr(now, 'HH:MM:SS'), chi, i, iii, T_arr(index), m_arr_2(index), corr_arr_2(index), marek_arr_2(index), ctr_arr_2(index), vumps_err_arr_2(index));

    end

    %save to main variables and delete temp dir
    saveboy( sprintf("%s.mat", name_prefix) , 'T_arr', 'm_arr', 'corr_arr', 'marek_arr', 'chi', 'J', 'g', 'vumps_err_arr', 'ctr_arr', T_arr, m_arr, corr_arr, marek_arr, chi, J, g, vumps_err_arr, ctr_arr);
